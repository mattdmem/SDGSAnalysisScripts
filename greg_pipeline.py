...
software = {"samtools": "/sdgs/software/samtools-1.3.1/samtools",
            ...
            "varscan": "/sdgs/software/varscan/VarScan.v2.4.3.jar",
...
def mapping(sample, out_dir, fastqs):
    ...
    fastq = " ".join(fastqs)
    command = [software["bwa"], "mem", resources["reference"], "-t", "24",
...
def coverage(sample, bam, out_dir):
    ...
    uniformity(sample, out_file, summary, out_dir)
    ...
    command = [software["sambamba"], "depth", "region", "--cov-threshold=0", "--cov-threshold=10", "--cov-threshold=15",
               "--cov-threshold=20", "--cov-threshold=30", "-o", out_file, "-c 0 -q 0 -L", resources["bed"], bam]
    run_command(" ".join(command))
...
def clean_variants(sample, vcf, out_dir):
    ...
    command = [software["vt"], "normalize", vcf, "-r", resources["reference"], "-o", out_file, "-n"]
    print " ".join(command)
    run_command(" ".join(command))
...
def find_true_germlines(sample, bam, out_dir):
    ...
    command = ["java", "-jar", software["gatk"], "-T", "HaplotypeCaller", "-R", resources["reference"], "-I", bam,
               "--output_mode", "EMIT_VARIANTS_ONLY", "-o", out_file, "-L", resources["bed"], "-stand_call_conf", "30",
               "-stand_emit_conf", "1"]
    run_command(" ".join(command))
...
def main():
    ...
    parser.add_argument('--fastqs_normal', metavar='fastq_normal', type=str, help='fastq for the normal')
